		...
		FOR_EACH_ENUM(Trait)
		{
			...
			if (kTrait.getCommerceChange(COMMERCE_CULTURE) > 0)
			{
				...
				if (pPersonality != NULL && pPersonality->getBasePeaceWeight() <= 5)
					m_bAmbitious = true;
				// advc.031: Disabled
				//m_iGreed += 15 * kTrait.getCommerceChange(COMMERCE_CULTURE);
			}
			...
			if (pPersonality != NULL && pPersonality->getMaxWarRand() <= 150) // advc.opt: moved up
			{
				for (int i = 0; i < kCiv.getNumUnits(); i++)
				{
						...
						kPlayer.canTrain(eFoundUnit))
					{
						// advc.031: Disabled
						/*m_iGreed += 20;
						if (pPersonality != NULL && pPersonality->getMaxWarRand() <= 150)*/
						m_bAmbitious = true;
					}
				}
			}
...
int CitySiteEvaluator::getGreed() const
{
	return m_iGreed;
}
...
void CitySiteEvaluator::log(int iX, int iY)
{
	if (isStartingLoc())
	{
		...
		{
			logBBAI("Normalizing starting plot (%d,%d)", iX, iY);
		}
		// About to be assigned
		else logBBAI("Starting location found at (%d,%d)", iX, iY);
	}
	else
	{
		logBBAI("About to found city at (%d,%d)", iX, iY);
...
short AIFoundValue::evaluate()
{
	...
	if (!isSitePromising() || !computeOverlap())
	{
	...
	for (int iPlot = 0; iPlot < NUM_CITY_PLOTS; iPlot++)
	{
		...
		if (!bHome && (!bRemovableFeature || iFeatureHealth > 0))
			iHealth += iFeatureHealth;

		BonusTypes const eBonus = getBonus(p);
		bool bCanTradeBonus = false;
		bool bCanSoonTradeBonus = false;
		ImprovementTypes eBonusImprovement = getBonusImprovement(
				eBonus, bCanTradeBonus, bCanSoonTradeBonus);

		int iCultureModifier = 100;
		if (!bHome) // advc.031: Shouldn't matter on home plot
		{
			calculateCultureModifier(p, bForeignOwned, bShare, bCityRadius,
					bSteal, abFlip[iPlot], bOwnExcl, iTakenTiles, iStealPercent);
		}
		...
		FOR_EACH_ENUM(Yield)
		{
			aiYield[eLoopYield] = //p->getYield(eLoopYield);  // K-Mod replacement:
			...
			if (bHome)
			{
				aiYield[eLoopYield] += p.calculateCityPlotYieldChange(
						eLoopYield, aiYield[eLoopYield], 1);
			} // </advc.031>
		}
		if (bHome)
		{
			iBaseProductionTimes100 += 100 * aiYield[YIELD_PRODUCTION];
			iSpecialFoodPlus += std::max(0, aiYield[YIELD_FOOD] -
					GC.getFOOD_CONSUMPTION_PER_POPULATION());
			/*adjustHomePlotYields(aiYield, eBonus, eBonusImprovement,
						bPersistentFeature ? eFeature : NO_FEATURE);*/
		}
		else
		{
			FOR_EACH_ENUM(Yield)
			{
				...
				if (kPlayer.getExtraYieldThreshold(eLoopYield) > 0 &&
					aiYield[eLoopYield] >= kPlayer.getExtraYieldThreshold(eLoopYield))
				{
					aiYield[eLoopYield] += GC.getDefineINT(CvGlobals::EXTRA_YIELD);
				} // </advc.031>
			}
			...
			if (!p.isWater())
			{
				...
				iPlotValue += evaluateFreshWater(p, aiYield, bSteal,
						iRiverTiles, iGreenTiles);
			}
		}
		iPlotValue += evaluateYield(p, aiYield); // (K-Mod: iTempValue in BtS)
		if (bHome)
			iValue += iPlotValue;
		else
		{
		...
		IFLOG logPlot(p, iPlotValue, aiYield, iCultureModifier, eBonus, eBonusImprovement, bCanTradeBonus,
				bCanSoonTradeBonus, bEasyAccess, iFeatureProduction, bPersistentFeature, bRemovableFeature);
		// ignore resources on hopelessly entrenched tiles
		// advc.031: Was 33, but now computed differently.
		if ((iCultureModifier <= 20 &&
			!bOwnExcl) || bSteal || bShare) // advc.035
		{
			IFLOG logBBAI("Plot unlikely to flip");
			continue;
		}
		iTotalFeatureProduction += iFeatureProduction; // advc.031 (after the culture check)
	...
	/*  K-Mod: try not to make the value of strategic resources too overwhelming.
		(note: I removed a bigger value reduction from the original code higher up.) */
	if (kSet.isStartingLoc())
	{
		...
		iResourceValue /=
				...
				(kGame.getStartingPlotNormalizationLevel() <= CvGame::NORMALIZE_LOW ? 3 : 4);
	}
	else if (iCities <= 0) // For moving the starting Settler
		iResourceValue = (2 * iResourceValue) / 3; // </advc.108>
	IFLOG if(iCities<=0) logBBAI("Non-yield resource value reduced for 1st city");
	...
	if (!kSet.isStartingLoc())
	{
		double foodModifier = calculateFoodModifier(iSpecialFoodPlus,
				iSpecialFoodMinus, iGreenTiles);
		/*  <advc.031> Don't apply the modifier to the non-yield resource value:
			don't need food for a trade connection. */
		iValue = ::round((iValue - iNonYieldResourceVal) * foodModifier) +
	...
	// <advc.031>
	if (iResourceValue <= 0 && iSpecialFoodPlus <= 0 && iRiverTiles < 4 &&
		iUnrevealedTiles < 5) // advc.040
	{
		double const mult = 0.6;
		iValue = ::round(mult * iValue);
		IFLOG logBBAI("Times %d percent because the site offers nothing special", ::round(100 * mult));
	} // </advc.031>
	...
	iValue = adjustToBadTiles(iValue, iBadTiles
...
// Sanity checks; mostly to avoid wasting time.
		...
		for (int i = 0; i < NUM_CITY_PLOTS; i++)
		{
			...
			if (p == NULL || (p->isOwned() && p->getTeam() != eTeam))
			{
				/*  advc.035 (comment): Would be good to check abFlip[i] here,
					but computeOverlap is a little costly, and I see the
					OwnedTiles check mostly as a timesaver. */
				iOwnedTiles++;
	...
	bool& bForeignOwned, bool& bAnyForeignOwned, bool& bShare, bool& bSteal) const
{
	...
	{
		IFLOG logBBAI("%S reserved for site at (%d,%d)", p->debugStr(),
...
/*  <advc.031> Rewritten. K-Mod had added a crucial isImprovementBonusTrade check, but other
	important checks were still missing. */
ImprovementTypes AIFoundValue::getBonusImprovement(BonusTypes eBonus, bool& bCanTrade,
	bool& bCanTradeSoon) const
{
	bCanTrade = bCanTradeSoon = false;
	if (eBonus == NO_BONUS)
	...
	FOR_EACH_ENUM(Build)
	{
		...
		{
			bCanTradeSoon = true;
			if (kTeam.isHasTech(eBuildPrereq))
				bCanTrade = true;
		}
		int iYieldValue = 0;
	...
	if (kSet.isStartingLoc())
	{
		...
		bCanTradeSoon = (bCanTradeSoon && isNearTech(eTradeTech));
		bCanTrade = (bCanTrade && kTeam.isHasTech(eRevealTech));
	} // </advc.108>
	...
	int& iTakenTiles, int& iStealPercent) const
{
	...
	r /= (iOtherCulture + kSet.getClaimThreshold()
			...
			(iOurCulture + kSet.getClaimThreshold())));
	// <advc.031>
...
/*  advc.031: I've come to think that this whole adjustment is unnecessary.
	Founding near a resource should naturally result in a higher found value
	than founding on top of a resource because of the higher (special) yields
	counted in the former case.
	Also, even if one assumes that founding on a resource needs to be explicitly
	discouraged, I don't think the K-Mod and AdvCiv code below gets it right. */
#if 0
void AIFoundValue::adjustHomePlotYields(int* aiYield, BonusTypes eBonus,
	ImprovementTypes eImprovement, FeatureTypes ePersistentFeature) const
{
	FOR_EACH_ENUM(Yield)
	{
		YieldTypes const eYield = eLoopYield; // abbreviate
		int& iYield = aiYield[eYield];
		// (this section has been rewritten for K-Mod. The original code was bork.)
		int iBasePlotYield = iYield;

		/*  (K-Mod) Subtract the special yield we'd get from a bonus improvement
			because we'd miss out on that by settling a city here.
			Exception: the improvement might be something dud which we wouldn't normally build.
			eg. +1 food from a plantation should not be counted, because
			a farm would be just as good.
			But +1 hammers from a mine should be counted, because we'd build the mine anyway.
			I haven't thought of a good way to deal with this issue. */
		if (eImprovement != NO_IMPROVEMENT)
		{
			//aiYield[eYield] -= GC.getInfo(eImprovement).getImprovementBonusYield(eBonus, eYield);
			// <advc.031> Replacing the above
			CvImprovementInfo& kImpr = GC.getInfo(eImprovement);
			int iImprBonusYield = kImpr.getImprovementBonusYield(eBonus, eYield);
			double const exp = 1.41;
			/*  Add this, exponentiate, then subtract it again. This should discourage
				the AI from settling on tiles with a high total yield.
				(E.g. 1 production lost from settling on Copper can't be evened out by
				fitting a plains hill instead of a desert hill into the BFC --
				the Copper would be worked much sooner than the plains hill!) */
			int iNonBonusImprYield = kImpr.getYieldChange(eYield) + iBasePlotYield;
			if (iImprBonusYield < 0) // (make sure not to exponentiate a negative base)
				iYield -= iImprBonusYield;
			else if (iImprBonusYield + iNonBonusImprYield > 0)
			{
				iYield -= ::round(std::pow((double)iImprBonusYield +
						iNonBonusImprYield, exp) - iNonBonusImprYield);
			}
		} // </advc.031>
		/*  (K-Mod) and subtract the base yield, so as to emphasise city plots
			which add yield rather than remove it.eg. plains-hill vs floodplain) */
		iYield -= iBasePlotYield;
	}
}
#endif
...
int AIFoundValue::evaluateYield(CvPlot const& p, int const* aiYield) const
{
	int r = 0;
	int aiWeight[NUM_YIELD_TYPES] = {15, 15, 8 };
	// (note: these numbers have been adjusted for K-Mod)
	if ((!isHome(p) || aiYield[YIELD_FOOD] >= GC.getFOOD_CONSUMPTION_PER_POPULATION()) &&
		!p.isWater()) // advc.031: Exclude seafood
	{
		r += 10;
	...
	FOR_EACH_ENUM(Yield)
	{
		...
		r += aiYield[eLoopYield] * aiWeight[eLoopYield];
	}
	if (p.isWater())
	{
	...
	bool bShare) const
{
	...
	else if (p.getOwner() == ePlayer ||
		stepDistance(iX, iY, p.getX(), p.getY()) <= 1)
	{
		r *= 3;
		r /= 2;
	}
	r *= kSet.getGreed(); // advc (note): Not really part of the culture adjustment
	r /= 100;
	// K-Mod end
	r *= iCultureModifier;
	r /= 100;
...
/*  Tbd.: Perhaps aiBonusCount should be computed upfront in a separate loop.
	As it is, it is only a partial count. */
int AIFoundValue::nonYieldBonusValue(CvPlot const& p, BonusTypes eBonus,
	bool bCanTrade, bool bCanTradeSoon, bool bEasyAccess,
	std::vector<int>& aiBonusCount, int iCultureModifier) const
{
	...
	// Coefficient was 80
	int r = ::round((kPlayer.AI_bonusVal(eBonus, 1, true) * 58.0) /
			(1 + aiBonusCount[eBonus]));
	// Note (K-Mod): the value of starting bonuses is reduced later.
	aiBonusCount[eBonus]++;
	r *= (kSet.isStartingLoc() ? 100 :
			// advc.031: Reduce impact of Greed (though it's disabled/ set to 100 anyway)
			(kSet.getGreed() + 200) / 3);
	r /= 100;
	/*  <advc.031> (cf. getBonusImprovement)
	...
	if (!bCanTrade)
	{
		FAssertMsg(kPlayer.getNumAvailableBonuses(eBonus) <= 0,
				"Should be implied by !bCanTrade");
		bool bSurplus = false;
		/*  The surplus check is important for high-value strategic resources
			that get revealed long before they can be traded, especially Oil. */
		if (!bSurplus)
		{
	...
	{	/*  <advc.031> Why halve the value of water bonuses? Perhaps because
		...
		FAssert(iCultureModifier <= 100);
		r = r * (kSet.isAmbitious() ? 110 : iCultureModifier) / 100;
	}
...
void AIFoundValue::calculateSpecialYields(CvPlot const& p,
	BonusTypes eBonus, ImprovementTypes eImprovement, int iCultureModifier,
	bool bEasyAccess, int iEffectiveFood, int* aiSpecialYield,
	int& iSpecialFoodPlus, int& iSpecialFoodMinus, int& iSpecialYieldTiles) const
{
	int iModifier = (bEasyAccess ? iCultureModifier : iCultureModifier / 2); // advc.040
	if (eBonus == NO_BONUS) // K-Mod: non bonus related special food
	{
		if (eImprovement == NO_IMPROVEMENT)
		{
			iSpecialFoodPlus +=
	...
	if (eImprovement == NO_IMPROVEMENT)
		return;
	CvImprovementInfo const& kImprovement = GC.getInfo(eImprovement);
	int iSpecialFood = p.calculateBestNatureYield(YIELD_FOOD, eTeam) +
			kImprovement.getImprovementBonusYield(eBonus, YIELD_FOOD);
	aiSpecialYield[YIELD_FOOD] += iSpecialFood;
	int iFoodTemp = iSpecialFood - GC.getFOOD_CONSUMPTION_PER_POPULATION();
	// advc.031:
	iFoodTemp = ::round(iFoodTemp * iCultureModifier / 100.0);
	iSpecialFoodPlus += std::max(0, iFoodTemp);
	iSpecialFoodMinus -= std::min(0, iFoodTemp);
	// <advc.031>
	int iSpecialProd = p.calculateBestNatureYield(YIELD_PRODUCTION, eTeam) +
			kImprovement.getImprovementBonusYield(eBonus, YIELD_PRODUCTION);
	int iSpecialComm = p.calculateBestNatureYield(YIELD_COMMERCE, eTeam) +
			kImprovement.getImprovementBonusYield(eBonus, YIELD_COMMERCE);
	...
	IFLOG if(bSpecial) logBBAI("Special yield: %dF%dP%dC", iSpecialFood, iSpecialProd, iSpecialComm);
	// </advc.031>
}
	...
	int iSpecialYieldTiles) const
{
	...
	int r = ::round(fromSpecial);
	IFLOG logBBAI("+%d from special yields (%dF%dP%dC)", r, aiSpecialYield[YIELD_FOOD],
			aiSpecialYield[YIELD_PRODUCTION], aiSpecialYield[YIELD_COMMERCE]);
	return r;
	// </advc.031>
}
...
double AIFoundValue::calculateFoodModifier(int iSpecialFoodPlus, int iSpecialFoodMinus,
	int iGreenTiles) const
{
	...
	double foodFactor = (100.0 + (kSet.isExpansive() ? 20 : 15) * std::min(
			(iFoodSurplus + iSpecialFoodPlus) / 2,
			2 * GC.getFOOD_CONSUMPTION_PER_POPULATION())) /
	...
	double lowFoodFactor = 1;
	if (eEra < 4)
	{
		lowFoodFactor = (7 + iGreenTiles + iSpecialFoodPlus -
	...
	double r = lowFoodFactor * foodFactor;
	IFLOG logBBAI("Food modifier: %d percent (special food +/-: %d/%d, green tiles: %d)",
			::round(100 * r), iSpecialFoodPlus, iSpecialFoodMinus, iGreenTiles);
	return r;
} // </advc.031>
	...
	int iLandTiles) const
{
	...
	/*	Apply greed only half. That said -
		Why is settling at the coast greedy at all? */
	/*int iHalfGreed = 100 + (kSet.getGreed() - 100) / 2;
	if (iHalfGreed > 100) {
		r *= iHalfGreed;
		r /= 100;
	}*/
...
int AIFoundValue::adjustToStartingSurroundings(int iValue) const
{
	...
	// (wip)
	#if 0 // advc.031: Disabled. This should be the responsibility of CvPlayer::findStartingAreas.
	{
		int const iTempValue = r; // advc.031c
		int iBonuses = 0;
		int iTypes = 0;
		int iPlayers = kArea.getNumStartingPlots();
		FOR_EACH_ENUM(Bonus)
		{
			int const iAreaBonuses = kArea.getNumBonuses(eLoopBonus);
			if (iAreaBonuses <= 0)
				continue; // advc
			iBonuses += 100 * iAreaBonuses / std::max(2, iPlayers + 1);
			iTypes += std::min(100, 100 * iAreaBonuses / std::max(2, iPlayers + 1));
		}
		// bonus for resources per player on the continent. (capped to avoid overflow)
		r = r * std::min(400, 100 + 2 * iBonuses/100 + 8 * iTypes/100) / 130;

		/*  penality for a solo start. bonus for pairing with an existing solo start.
			penality for highly populated islands */
		r = r * (10 + std::max(-2, iPlayers > 0 ? 2 - iPlayers : -2)) / 10;
		/*if (iPlayers == 0)
			r = r * 85 / 100;
		else if (iPlayers == 1)
			r = r * 110 / 100;*/
		IFLOG if(r!=iTempValue) logBBAI("%d from player density", r - iTempValue);
	} // K-Mod end
	#endif
...
int AIFoundValue::adjustToProduction(int iValue, int iBaseProductionTimes100) const
{
	...
	if (iBaseProductionTimes100 < 100 * iThreshold)
	{
		iValue = ::round(iValue * iBaseProductionTimes100 / (100 * iThreshold)); // advc
		IFLOG logBBAI("Times %d percent for low production (%d/100)",
				::round(iBaseProductionTimes100 / (double)iThreshold), iBaseProductionTimes100);
	} // K-Mod end
	return iValue;
}
...
int AIFoundValue::adjustToCivSurroundings(int iValue, int iStealPercent) const
{
	...
	if (pOurNearestCity != NULL)
	{
		...
		IFLOG if(iTempValue!=iValue) logBBAI("%d from distance to %S", iValue - iTempValue, cityName(*pOurNearestCity));
	...
	{
		int iDistPenalty = 7000 - eEra * 1000; // (was 8000 flat)
...
int AIFoundValue::adjustToCitiesPerArea(int iValue) const
{
	...
	/* (remove this comment delimiter to re-enable the modified BtS code)
		...
		int iTeamAreaCities = kTeam.countNumCitiesByArea(&kArea);
		//if (kArea.getNumCities() == iTeamAreaCities)
		// advc.040: Don't let a single rival city make all the difference
		if(7 * kArea.getNumCities() < 10 * iTeamAreaCities) {
			// advc.040: Why care about Barbarian cities? And times 3/2 is huge.
			//iValue *= 3; iValue /= 2;
		//}
		//else if (kArea.getNumCities() == (iTeamAreaCities + GET_TEAM(BARBARIAN_TEAM).countNumCitiesByArea(&kArea))) {
...
int AIFoundValue::adjustToBadTiles(int iValue, int iBadTiles) const
{
	...
	if(iBadTiles > 0)
	{
		r -= ::round(std::pow((double)iBadTiles, 1.33) *
...
int AIFoundValue::adjustToBadHealth(int iValue, int iGoodHealth) const
{
	...
	{
		int const iDiv = std::max(1, (3 - eEra + iBadHealth));
		iValue /= iDiv;
		IFLOG if (iDiv>1) logBBAI("Times 1/%d for bad health", iDiv);
	}
	return iValue;
} // </advc.031>
...
bool AIFoundValue::isDeadlockedBonus(CvPlot const& kBonusPlot, int iMinRange) const
{
	...
	for(int iPlot = 0; iPlot < NUM_CITY_PLOTS; iPlot++)
	{
		...
		if(p == NULL ||
				isHome(*p)) // advc.031: Won't want to settle on top of the bonus
			continue;
	...
	int iFeatureProduction, bool bPersistentFeature, bool bRemovableFeature) const
{
	...
	if (iFeatureProduction != 0 && !isHome(p))
	{
		FAssert(!bPersistentFeature);
		logBBAI("Feature production: %d", iFeatureProduction);
		if (!bRemovableFeature)
			logBBAI("(reduced b/c can't remove yet)");
	}
