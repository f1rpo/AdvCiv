...
/************************************************************************************************/           
			...
			else if (pPlot->area()->getAreaAIType(pPlot->getTeam()) == AREAAI_ASSAULT_ASSIST)
...
/************************************************************************************************/
	...
	else
	{
		...
		if(u != NULL && u->isFound() && pPlot->getFeatureType() != NO_FEATURE) {
			...
			if(healthPercent != 0) {
				szString.append(NEWLINE);
				float health = abs(healthPercent / 100.0f);
				int icon = (healthPercent > 0 ? gDLL->getSymbolID(HEALTHY_CHAR) :
		...
		if (eImprovement != NO_IMPROVEMENT)
		{
			szString.append(NEWLINE);
			// <advc.005c>
			bool namedRuin = false;
			if(eImprovement == (ImprovementTypes)GC.getDefineINT("RUINS_IMPROVEMENT")) {
			...
			if(!namedRuin) // from Volcano event // </advc.005c>
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText, bool bStrategyText, bool bTechChooserText, CvCity* pCity)
{
	...
	if(pCity != NULL && pCity->plot() != NULL &&
			GC.getUnitInfo(eUnit).getAirUnitCap() > 0 && pCity->plot()->
...
// BUG - Building Actual Effects - end
{
	...
	// And removed ePlayer==NO_PLAYER checks in later code
	if(bct == NO_BUILDINGCLASS || ePlayer == NO_PLAYER)
		return;
...
*/
		...
		if (pCity)
		{
			...
			{
				if (kPlayer.isCivic((CivicTypes)i))
				{
...
*/
				if (ePlayer != NO_PLAYER &&
					...
					kBuilding.getReligionType() == kPlayer.getStateReligion())
				{
					aiCommerces[iI] += kPlayer.getStateReligionBuildingCommerce((CommerceTypes)iI);
				}
/*
** K-Mod end
*/
		...
		{	// <advc.004w>
			...
			if(pCity != NULL) {
				...
				if(tech != NO_TECH && GC.getTechInfo(tech).getEra() -
						kPlayer.getCurrentEra() > 1)
					continue;
				if(!kPlayer.isCivic(ct))
					parentheses = true;
			} // </advc.004w>
...
/************************************************************************************************/
			...
			{
				BuildingTypes ePlayerBuilding = ((BuildingTypes)(GC.getCivilizationInfo(kPlayer.getCivilizationType()).getCivilizationBuildings(iI)));
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
	{
		...
		if (ePlayer != NO_PLAYER)
		{
			eLoopBuilding = ((BuildingTypes)(GC.getCivilizationInfo(kPlayer.getCivilizationType()).getCivilizationBuildings(iI)));
		}
		else
		{
			eLoopBuilding = (BuildingTypes)GC.getBuildingClassInfo((BuildingClassTypes)iI).getDefaultBuildingIndex();
							...
							sense than getNumRealBuilding. */
	{
		if (!bCivilopediaText)
		{
			if (pCity == NULL)
			{
				...
				{
					szTempBuffer.Format(L"\n%d%c", kPlayer.getProductionNeeded(eBuilding), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
					szBuffer.append(szTempBuffer);
				}
			}
			else
	...
	if (bStrategyText)
	{
		...
		{
			if (kPlayer.isOption(PLAYEROPTION_ADVISOR_HELP))
			{
				szBuffer.append(SEPARATOR);
				szBuffer.append(NEWLINE);
				szBuffer.append(gDLL->getText("TXT_KEY_SIDS_TIPS"));
